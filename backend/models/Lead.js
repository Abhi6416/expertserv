 ...
 * Updated: Removed whatsappSent field (WhatsApp notifications removed).
 */
...
const leadSchema = new mongoose.Schema(
  {
    ...
    email:       { type: String, required: [true, "Email required"],       trim: true, lowercase: true,
    ...
    companyName: { type: String, required: [true, "Company name required"], trim: true, maxlength: 200 },
    state:       { type: String, required: [true, "State required"],        trim: true },
    solution:    { type: String, required: [true, "Solution required"],
                   enum: { values: ["IVR","RCS","SMS","OTP"], message: "Must be IVR, RCS, SMS, or OTP" } },
    preferredDate: { type: Date,   required: [true, "Preferred date required"] },
    preferredTime: { type: String, required: [true, "Preferred time required"] },
    agreedToPolicy:{ type: Boolean, required: true,
                   validate: { validator: v => v === true, message: "Must agree to policy" } },
    status:      { type: String, enum: ["new","contacted","converted","closed"], default: "new" },
    ipAddress:   { type: String,  default: null },
    emailSent:   { type: Boolean, default: false },
    // whatsappSent removed — WhatsApp notifications no longer used
  },
  { timestamps: true }
);
